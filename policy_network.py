...
class InfoRolloutBuffer(RolloutBuffer):
    def __init__(
        ...
        super().__init__(
            buffer_size, observation_space, action_space, device, gamma, gae_lambda
        )
        ...
        while n_steps < n_rollout_steps:
            ...
            if not dones[0]:
                assert np.array_equal(
                    env.envs[0].env.current_env_params, infos[0]["current_env_params"]
                ), (
                    f"Env params should be the same: {env.envs[0].env.current_env_params} != {infos[0]['current_env_params']}"
                )
