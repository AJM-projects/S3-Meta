...
class DelayedMABEnv(gym.Env):
    ...
    def __init__(
        ...
        n_bandits=5,
        signal1_length=25,
        distractor1_length=15,
        intermediate_decision_length=10,
        signal2_length=25,
        distractor2_length=15,
        final_decision_length=10,
        ...
        total_length = (
            signal1_length
            + distractor1_length
            + intermediate_decision_length
            + signal2_length
            + distractor2_length
            + final_decision_length
        )
        ...
        self.intermediate_decision_end = (
            self.distractor1_end + intermediate_decision_length
        )
        ...
        self.signal1_cumulative = (
            0.0  # Track signal 1 separately for intermediate decision
        )
    ...
    def reset(self, seed=None, **kwargs):
        ...
        self.multipliers = np.random.choice(
            [-1, 1], self.n_bandits
        )  # Random +1 or -1 multipliers
    ...
    def step(self, action):
        ...
        # Episode termination
        if (
            self.current_step >= self.final_decision_end
